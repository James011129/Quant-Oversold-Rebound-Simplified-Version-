...
def initialize(context):
    ...
    log.info("策略初始化完成。最大持仓: %d只，策略模式：无未来函数 - 下跌止跌回升 + 多因子评分。" % g.max_hold)
...
def trade(context):
    ...
    if buy_list:
        log.info(f"【选股结果】今日得分排名前{len(buy_list)}的标的：{buy_list}")
    else:
    ...
    for stock in list(context.portfolio.positions.keys()):
        if stock not in target_positions:
            log.info(f"🟡【调仓卖出】发起对 {stock} 的全额平仓委托。")
...
def check_stocks(context):
    ...
    # 【修改点 1】：改为使用上一交易日 (T-1) 日期，避免开盘获取未收盘的当日数据
    date = context.previous_date

    # 全部A股 (基于上一交易日筛选)
    all_stocks = get_all_securities(['stock'], date=date).index.tolist()

    # 1. 第一层过滤：剔除创业板/科创板/北交所及今日停牌股票
    target_list = []
    for stock in all_stocks:
        if stock.startswith('30') or stock.startswith('68') or stock.startswith('9') or stock.startswith('8'):
            continue
        # 今日开盘停牌则过滤
    ...
    for stock in valid_stocks:
        try:
            # 【修改点 2】：改用 get_bars 并且设置 include_now=False
            # 确保获取的数据彻底排除 T 日（今日），只保留截至 T-1 日（昨日）的收盘数据
            hist = get_bars(
                stock, count=40, unit='1d',
            ...
            # ===========================
            # 核心逻辑 2：定义“止跌回升”
            # iloc[-1]: T-1日, iloc[-2]: T-2日, iloc[-3]: T-3日
            # ===========================
